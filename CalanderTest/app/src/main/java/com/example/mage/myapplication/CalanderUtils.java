...
//        values.put(CalendarContract.Reminders.MINUTES, previousDate * 24 * 60);// 提前previousDate天有提醒
    ...
    public static void updateCalenderEvent(Context context,String title,String description){
        if (context == null) {
            return;
        }
        Cursor eventCursor = context.getContentResolver().query(Uri.parse(CALENDER_EVENT_URL), null, null, null, null);
        try {
            ...
            Uri updateUri = Uri.parse(CALENDER_EVENT_URL);

            context.getContentResolver().update(updateUri,contentValues,"title=?",new String[]{title});
    ...
    public static String queryCalenderEvent(Context context, String title){
        if (context == null) {
            return title;
        }
        JSONArray jo = new JSONArray();

        Cursor eventCursor = context.getContentResolver().query(Uri.parse(CALENDER_EVENT_URL), null, "title=?", new String[]{title}, null);
        try {
            if (eventCursor == null) { //查询返回空值
                return null;
            }
            if (eventCursor.getCount() > 0) {
                //遍历所有事件，找到title跟需要查询的title一样的项
                for (eventCursor.moveToFirst(); !eventCursor.isAfterLast(); eventCursor.moveToNext()) {
                    String queryTitle = eventCursor.getString(eventCursor.getColumnIndex("title"));//取得id
                    String queryDesc = eventCursor.getString(eventCursor.getColumnIndex("description"));
                    JSONObject map = new JSONObject();
                    try {
                        map.put("title",queryTitle);
                        map.put("description",queryDesc);
                        jo.put(map);
